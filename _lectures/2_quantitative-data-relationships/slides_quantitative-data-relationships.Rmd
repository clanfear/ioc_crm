...
date: "18 Oct 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%d %b %Y'))`"
output:
...
**Unobservables** (or **Constructs**): Subjective, internal, or only indirectly measurable
   ...
   * Individual self-control, wellness, social class, education
...
* If something is 0cm long, it has *no length*

* If the temperature is 0C, there isn't *no temperature*
   * The 0 is arbitrary: In Fahrenheit it is 32 degrees!

--

* Multiplicative differences need ratio data
...
This means you cannot add/subtract or multiple/divide ordinal data
...
.footnote[[1] For continuous data, the mode is a *bit different* but is basically the same]
...
These all have the same mean and standard deviation!
...
dists %>%
  ...
  knitr::kable(col.names = c("Name", "Mean", "Median", "Std. Dev"))
...
* The probability of $Y$ taking a particular value(s) when $X$ takes a particular value(s)
...
Visualisation is important—these all have the same correlation!
...
* No Wednesday drop-in hours due to faculty meeting