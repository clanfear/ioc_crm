...
date: "28 Oct 2025<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%d %b %Y'))`"
output:
...
communities <- communities_orig |>
  ...
  mutate(across(c(Disadvantage, Incarceration), ~factor(., levels = c("Low", "Medium", "High"))))
```
...
r_df <- map_dfr(setNames(r_seq, r_seq), ~ make_sample(., 100), .id = "R") |>
  mutate(R = factor(R, levels = r_seq),
         R = fct_relabel(R, \(x) str_c("r=", x, "\nExplained: ", round(100*(as.numeric(x)^2)), "%"))) 
...
form <- y ~ x
communities %>%
  mutate(pred = predict(lm(`Crime Rate`~`Pop Density`, data = .))) |>
  ggplot(aes(x = `Pop Density`, y = `Crime Rate`)) + 
  xlab("Population Density") +
  stat_fit_deviations(formula = form, color = "red") +
  geom_segment(aes(x = `Pop Density`, y = `Crime Rate`, yend = pred), color = "red", size = 0.5) +
  geom_point(alpha = 0.5) +
...
ggplot(draws, aes(x = Heads, y = Probability, fill = `95%`)) + 
  geom_col() + 
  scale_fill_manual(values = c("#003C71","#8A1538")) +
  scale_x_continuous(breaks = 0:20) +
  theme_minimal(base_size = 16) +
  labs(x = "Number of heads out of 20") +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
    plot.background = element_rect(fill='transparent', color=NA),
    panel.grid.major = element_line(color = "black", size = 0.1),
...
5% or less of the time would we see 5 or fewer or 15 or more.
...
Cross-tabs are used to show associations between categorical variables
...
What does no association look like?

```{r}

tibble(
  Sex = rep(c("Male","Female"), length.out = 120),
  `Self-Control` = c(rep("Low", 30), rep("Medium", 50), rep("High", 40))
)  |> 
  tabyl(`Self-Control`, Sex) |>
  adorn_title() |>
  knitr::kable()
```
...
*Males and females are equally probable of being in each level of self-control*


--

But we could get different probabilities just from randomness in sampling.

---

# Chi-Square Test

The chi-square ( $\chi^2$ ) asks: How likely would we be to see a sample with this strong an association if the there were no relationship in the population?
...
*Disadvantage and Area are more strongly related in this sample than is likely to occur due only to sampling noise.*
...
beta_draws <- rbeta(100000, 1.5, 1)
beta_mean_draws <- map_dbl(1:100000, ~mean(sample(beta_draws, 100)))
...
```{r, fig.height = 4}
tibble(beta_draws) |>
  ggplot(aes(x = beta_draws)) + 
  geom_histogram(aes(y = (..count..)/sum(..count..)), bins = 50, fill = "black") + 
  theme_minimal(base_size=16) +
  labs(x = "X", y = "Probability") +
  geom_vline(aes(xintercept = mean(beta_draws)), color = "red") +
  annotate("text", x = mean(beta_draws)+0.07, y= Inf, vjust = 2, label = sprintf("%.2f", round(mean(beta_draws),2)), color = "red") +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
    plot.background = element_rect(fill='transparent', color=NA),
    panel.grid.major = element_line(color = "black", size = 0.1)
...
```{r, fig.height = 4}
tibble(beta_draws = sample(beta_draws, 100)) |>
  ggplot(aes(x = beta_draws)) + 
  geom_dotplot(binwidth = 0.02, method = "histodot", fill = "black") + 
  theme_minimal() +
  scale_x_continuous(limits = c(0,1)) +
  geom_vline(aes(xintercept = mean(beta_draws)), color = "red") +
  scale_y_continuous(breaks = seq(0, 1, 0.195), labels = 5*(seq_along(seq(0, 1, 0.195)) -1)) +
  annotate("text", x = mean(beta_draws)+0.09, y= Inf, vjust = 2, label = sprintf("%.2f", round(mean(beta_draws),2)), color = "red") +
  theme_minimal(base_size = 16) +
  labs(x = "X", y = "N") +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
...
Our estimate is great with only $N=100$! But if we couldn't compare to the population, how would we know that?

---

# Many Samples

If we take a ton of samples (n=100 each) and calculate all their means, their distribution looks like this—most are pretty accurate: 95% of them are between 0.55 and 0.65!

```{r, fig.height = 4}

tibble(beta_mean_draws) |>
  ggplot(aes(x = beta_mean_draws)) + 
  geom_histogram(aes(y = ..count../sum(..count..)), bins = 50, fill = "black") + 
  geom_vline(aes(xintercept = mean(beta_mean_draws)), color = "red")  +
  annotate("text", x = mean(beta_mean_draws)+0.015, y= Inf, vjust = 2, label = sprintf("%.2f", round(mean(beta_mean_draws),2)), color = "red") +
  labs(y = "Probability", x = expression(bar(X))) +
  theme_minimal(base_size = 16) +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
    plot.background = element_rect(fill='transparent', color=NA),
...
tibble(norm_draws) |>
  ...
  annotate("text", x = mean(norm_draws)+2.5, y= Inf, vjust = 2, label = sprintf("%.2f", round(mean(norm_draws),2)), color = "red") +
  labs(x = "X", y = "Probability") +
  theme_minimal(base_size = 16) +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
...
tibble(norm_mean_draws) |>
  ...
  labs(y = "Probability", x = expression(bar(X))) +
  annotate("text", x = mean(norm_mean_draws)+0.5, y= Inf, vjust = 2, label = sprintf("%.2f", round(mean(norm_mean_draws),2)), color = "red") +
  theme_minimal(base_size = 16) +
  theme(
    panel.background = element_rect(fill='transparent', color=NA), 
    plot.background = element_rect(fill='transparent', color=NA),
...
[Confidence interval example](https://rpsychologist.com/d3/ci/)
...
The small standard errors translate into a narrow confidence interval around the line:
...
Note it is not an interval containing 95% of the *data points*
